...
"""
    Code for Symmetric Flow Attractor.
"""

simulation_name = "SymmetricFlow"
...
for t in time:
    dx_dt = -a*x_new - 4*y_new - 4*z_new - y_new*y_new
    dy_dt = -a*y_new - 4*z_new - 4*x_new - z_new*z_new
    dz_dt = -a*z_new - 4*x_new - 4*y_new - x_new*x_new
...
if (True):
    ...
    ax.plot(x, y, z, linewidth=0.09, color='black')
...
if (False):
    ...
    trace = Line3DCollection(dummy_seg, linewidth=0.1)
    trace.set_alpha(0.9)
